 ...
 */
static pte_t *create_page_table(void) {
    if (!kernel_page_table) {
        kernel_page_table = (pte_t *) alloc_pages(1);
    ...
    pte_t *page_table = (pte_t *) alloc_pages(1);
 ...
 */
static struct process *alloc_process(uintptr_t entry) {
...
struct process *create_idle_process(void) {
    ...
    struct process *proc = alloc_process((uintptr_t) idle_entry);
    printf("page_table=0x%lx\n", (uintptr_t) proc->page_table);
    return proc;
}
...
struct process *create_process2(const void *image, size_t image_size,
    ...
    struct process *proc = alloc_process((uintptr_t) user_entry);
...
void yield(void) {
    ...
    __asm__ __volatile__(
        ...
        :
        : [satp] "r" (SATP_MODE | ((uintptr_t) next->page_table / PAGE_SIZE)),
